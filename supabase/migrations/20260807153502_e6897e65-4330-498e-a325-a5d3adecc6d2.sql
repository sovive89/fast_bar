...
GRANT SELECT (id, customer_name, phone, status, started_at, closed_at, paid_at, created_at) ON public.bar_sessions TO anon, authenticated;
GRANT ALL ON public.bar_sessions TO service_role;
ALTER TABLE public.bar_sessions ENABLE ROW LEVEL SECURITY;
CREATE POLICY "sessions readable by everyone" ON public.bar_sessions FOR SELECT USING (true);
...
GRANT SELECT ON public.bar_tab_items TO anon, authenticated;
GRANT ALL ON public.bar_tab_items TO service_role;
ALTER TABLE public.bar_tab_items ENABLE ROW LEVEL SECURITY;
CREATE POLICY "tab items readable by everyone" ON public.bar_tab_items FOR SELECT USING (true);
